...
pub async fn get_svg_image(
    ...
    println!("{now} webpage_hit_counter/get_svg_image/{webpage_id}.svg");

    // When I use preview in VSCode it makes hundreds of request to the server and
    // this increments the counter enormously. I don't want that. I will check for Electron and avoid the increment.
    ...
    let hit_count = crate::postgres_mod::select_count(db_pool, webpage_id, do_increment).await.unwrap();
